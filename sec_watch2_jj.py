...
def block_ip_with_ufw(ip):
    if ip in blocked_ips:
        ...
        try:
            subprocess.run(["sudo", "ufw", "deny", "from", ip, "to", "any"], check=True, stdout=subprocess.DEVNULL)
            print(f"Blockerade IP: {ip} med UFW.")
            blocked_ips.add(ip)  # Lägg till i den interna listan
        except subprocess.CalledProcessError:
            print(f"Misslyckades med att blockera IP: {ip}")
